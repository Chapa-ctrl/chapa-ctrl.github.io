...
const jokes = [
  ...
  "❄️ That guess was colder than ice cream.",
  "🤓 Even Einstein was wrong sometimes. Try again!",
  "🙃 Wrong number, but you’re getting good practice!"
];
...
function checkGuess() {
  ...
  if (guess === randomNumber) {
    ...
    message.innerText = `🎉 Correct! You guessed it in ${guessCount} tries and ${timeTaken}s!`;
    document.getElementById("reset-btn").style.display = "inline-block";
    saveScore(currentPlayer, guessCount, timeTaken);
  } else if (guessCount >= 15) {
    clearInterval(timerInterval);
    message.innerText = `😅 Too many tries! The number was ${randomNumber}. Numbers aren’t easy, huh?`;
    document.getElementById("reset-btn").style.display = "inline-block";
  } else {
...
function checkScreenSize() {
  if (window.innerWidth <= 768) {
    document.getElementById("toggle-leaderboard").style.display = "inline-block";
    document.getElementById("leaderboard-container").style.display = "none";
  } else {
    document.getElementById("toggle-leaderboard").style.display = "none";
